...
class FillReconciler:
    def reconcile(self,target_qty:Decimal,buy_filled:Decimal,sell_filled:Decimal,tolerance:Decimal=Decimal("0"))->Reconciliation:
        target=Decimal(str(target_qty)); buy=max(Decimal(0),Decimal(str(buy_filled))); sell=max(Decimal(0),Decimal(str(sell_filled))); residual=buy-sell
        if buy>target or sell>target:return Reconciliation(ReconciliationState.FAILED,buy,sell,residual,True,"overfill")
        if abs(residual)<=Decimal(str(tolerance)) and buy>=target and sell>=target:return Reconciliation(ReconciliationState.BALANCED,buy,sell,residual,False,"balanced")
