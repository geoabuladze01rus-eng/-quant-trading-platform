...
class BacktestEngine:
    def __init__(self,fee_bps=Decimal("10"),slippage_bps=Decimal("0"),min_edge_bps=Decimal("0")):
        self.fee_bps=Decimal(str(fee_bps)); self.slippage_bps=Decimal(str(slippage_bps)); self.min_edge_bps=Decimal(str(min_edge_bps))
    def run(self,events:Iterable[BacktestEvent],signal:Callable[[BacktestEvent],bool]|None=None)->BacktestResult:
        ...
        for e in events:
            ...
            if edge<self.min_edge_bps or (signal and not signal(e)): continue
